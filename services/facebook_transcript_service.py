...
ASR_TIMEOUT_SECONDS = int(os.getenv("FACEBOOK_ASR_TIMEOUT_SECONDS", "180"))
ASR_MAX_BYTES = int(os.getenv("FACEBOOK_ASR_MAX_BYTES", str(24 * 1024 * 1024)))
ASR_RAW_MAX_BYTES = int(os.getenv("FACEBOOK_ASR_RAW_MAX_BYTES", str(128 * 1024 * 1024)))
ASR_FFMPEG_TIMEOUT_SECONDS = int(os.getenv("FACEBOOK_ASR_FFMPEG_TIMEOUT_SECONDS", "180"))
...
FACEBOOK_IMPERSONATE_TARGET = os.getenv("FACEBOOK_IMPERSONATE_TARGET", "chrome")
FACEBOOK_APP_INFO = os.getenv(
    "FACEBOOK_APP_INFO",
    "",
).strip()
FACEBOOK_ENABLE_YTDLP_CLI_FALLBACK = os.getenv("FACEBOOK_ENABLE_YTDLP_CLI_FALLBACK", "1") == "1"
FACEBOOK_YTDLP_CLI_TIMEOUT_SECONDS = int(os.getenv("FACEBOOK_YTDLP_CLI_TIMEOUT_SECONDS", "120"))

_facebook_info_cache: Dict[str, Tuple[float, dict]] = {}
_facebook_subtitle_cache: Dict[str, Tuple[float, dict]] = {}
_AUDIO_EXTENSIONS = {"mp3", "wav", "flac", "ogg", "opus", "webm", "m4a", "aac"}


class FacebookTranscriptError(Exception):
    def __init__(
        self,
        code: str,
        message: str,
...
def _error_with_stage(
    ...
    return FacebookTranscriptError(err.code, err.message, err.status_code, details)


def normalize_facebook_url(raw_url: str) -> str:
    if not isinstance(raw_url, str) or not raw_url.strip():
        raise FacebookTranscriptError("INVALID_URL", "A valid URL is required.", 400)
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise FacebookTranscriptError("INVALID_URL", "Invalid URL format.", 400)

    host = parsed.netloc.split(":")[0].lower()
    if ("facebook.com" not in host) and (not host.endswith("fb.watch")):
        raise FacebookTranscriptError(
            "UNSUPPORTED_PLATFORM",
            "Only Facebook URLs are supported for this endpoint.",
            400,
        )
...
def _map_extraction_error(exc: Exception) -> FacebookTranscriptError:
    message = str(exc)
    lower_message = message.lower()

    if "private" in lower_message or "login" in lower_message:
        return FacebookTranscriptError(
            "PRIVATE_OR_LOGIN_REQUIRED",
            "This Facebook video is private or requires login.",
            403,
        )
    if "geo" in lower_message or "country" in lower_message or "region" in lower_message:
        return FacebookTranscriptError(
            "GEO_RESTRICTED",
            "This Facebook video is not available in your region.",
            403,
        )
    if "not available" in lower_message or "unavailable" in lower_message:
        return FacebookTranscriptError(
            "VIDEO_UNAVAILABLE",
            "This Facebook video is unavailable.",
            404,
    ...
    if "impersonate" in lower_message and "unsupported" in lower_message:
        return FacebookTranscriptError(
            "VIDEO_UNAVAILABLE",
            "Failed to load this Facebook video right now.",
            502,
            {"raw_error": message[:500]},
        )
    ...
    return FacebookTranscriptError(
        "VIDEO_UNAVAILABLE",
        "Failed to fetch Facebook video metadata.",
        502,
        {"raw_error": message[:500]},
    )
...
def _get_info_proxy_url() -> Optional[str]:
    env_proxy = os.getenv("FACEBOOK_INFO_PROXY_URL", "").strip()
    if env_proxy:
        return env_proxy

    if os.getenv("FACEBOOK_ENABLE_BUILTIN_PROXY", "1") != "1":
        return None
...
def _ydl_options(
    ...
    if use_impersonate:
        opts["impersonate"] = FACEBOOK_IMPERSONATE_TARGET

    if use_app_info and FACEBOOK_APP_INFO:
        opts["extractor_args"] = {"facebook": {"app_info": [FACEBOOK_APP_INFO]}}
...
def _extract_info_via_ytdlp_cli_once(
    ...
    cli_cmd = [
        ...
        "",
        "--impersonate",
        FACEBOOK_IMPERSONATE_TARGET,
        "--dump-single-json",
        "--skip-download",
        normalized_url,
    ]
    if use_app_info and FACEBOOK_APP_INFO:
        cli_cmd.extend(["--extractor-args", f"facebook:app_info={FACEBOOK_APP_INFO}"])
    ...
    try:
        completed = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=FACEBOOK_YTDLP_CLI_TIMEOUT_SECONDS,
            check=False,
        )
...
def _extract_info_via_ytdlp_cli(normalized_url: str) -> dict:
    last_error: Optional[FacebookTranscriptError] = None
...
def _extract_info(normalized_url: str, force_refresh: bool = False) -> dict:
    cache_key = f"facebook:{normalized_url}"
    if not force_refresh:
        cached = _cache_get(_facebook_info_cache, cache_key, INFO_CACHE_TTL)
    ...
    if info is None and FACEBOOK_ENABLE_YTDLP_CLI_FALLBACK:
        try:
            info = _extract_info_via_ytdlp_cli(normalized_url)
        except FacebookTranscriptError as cli_err:
            last_error = cli_err
    ...
    if not isinstance(info, dict):
        raise FacebookTranscriptError(
            "VIDEO_UNAVAILABLE",
            "Failed to read Facebook video metadata.",
            502,
        )

    _cache_set(_facebook_info_cache, cache_key, info)
    return info
...
def _resolve_language(requested_lang: Optional[str], available_codes: List[str]) -> Tuple[str, List[str]]:
    if not available_codes:
        raise FacebookTranscriptError(
            "NO_SUBTITLE",
            "No subtitles are available for this Facebook video.",
            404,
        )
...
def _sanitize_filename(value: str) -> str:
    ...
    if not safe:
        safe = "facebook_transcript"
    return safe[:120]


def _subtitle_cache_key(normalized_url: str, lang: str, output_type: str) -> str:
    return f"facebook:{normalized_url}:{lang}:{output_type}"
...
def _download_media_no_proxy(media_candidate: dict) -> Tuple[bytes, str]:
    ...
    if not media_url:
        raise FacebookTranscriptError(
            ...
            "Audio media URL is unavailable for this Facebook video.",
            404,
        )
    ...
    # Prefer curl_cffi with browser impersonation to reduce facebook CDN 403 on direct broadband.
    if curl_requests is not None:
        try:
            response = curl_requests.get(
                ...
                headers=headers,
                timeout=45,
                impersonate=FACEBOOK_IMPERSONATE_TARGET,
                proxies={},
            )
    ...
    try:
        with session.get(media_url, headers=headers, timeout=45, stream=True) as response:
            ...
            media_ext = str(fmt.get("ext") or "mp4").lower()
            return media_bytes, media_ext
    except FacebookTranscriptError:
        raise
...
def _download_media_via_ytdlp_no_proxy_once(
    ...
    with tempfile.TemporaryDirectory(prefix="facebook_media_") as temp_dir:
...
def _download_media_via_ytdlp_cli_no_proxy(info: dict, media_candidate: dict) -> Tuple[bytes, str]:
    ...
    for selector in selectors:
        for use_app_info in (True, False):
            with tempfile.TemporaryDirectory(prefix="facebook_media_cli_") as temp_dir:
                ...
                try:
                    ...
                    last_error = FacebookTranscriptError(
                        "ASR_MEDIA_UNAVAILABLE",
...
def _download_media_with_fallback(info: dict, media_candidate: dict) -> Tuple[bytes, str]:
    direct_error: Optional[FacebookTranscriptError] = None
    ytdlp_py_error: Optional[FacebookTranscriptError] = None
    ytdlp_cli_error: Optional[FacebookTranscriptError] = None
    ...
    if FACEBOOK_ENABLE_YTDLP_CLI_FALLBACK:
        try:
...
def _normalize_media_for_asr(media_bytes: bytes, media_ext: str) -> Tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="facebook_asr_media_") as temp_dir:
...
def get_facebook_info_payload(url: str, preferred_lang: Optional[str] = None) -> dict:
...
def _build_raw_bundle(info: dict, requested_lang: Optional[str]) -> Optional[dict]:
    ...
    return {
        "title": info.get("title") or "facebook_transcript",
        "thumbnail": info.get("thumbnail"),
        "video_meta": _build_video_metadata(info),
        "lang_used": lang_used,
        "fallback_order": fallback_order,
...
def _build_asr_bundle(info: dict, requested_lang: Optional[str]) -> dict:
    ...
    file_name = f"{_sanitize_filename(str(info.get('id') or 'facebook'))}.{media_ext}"
    ...
    try:
        ...
        if isinstance(exc, FacebookTranscriptError):
...
def _build_content_bundle(normalized_url: str, requested_lang: Optional[str]) -> dict:
    ...
    try:
    ...
    except FacebookTranscriptError as first_err:
        ...
        except FacebookTranscriptError as second_err:
...
def get_facebook_content_payload(url: str, lang: Optional[str] = None) -> dict:
    normalized_url = normalize_facebook_url(url)
    requested_lang = (lang or "en").strip() or "en"
    cache_key = _subtitle_cache_key(normalized_url, requested_lang, "content")
    cached = _cache_get(_facebook_subtitle_cache, cache_key, SUBTITLE_CACHE_TTL)
    if cached is not None:
        return cached
    ...
    _cache_set(_facebook_subtitle_cache, cache_key, payload)
    return payload


def get_facebook_download_payload(url: str, lang: Optional[str], output_type: str) -> Tuple[bytes, str, str]:
    normalized_url = normalize_facebook_url(url)
    requested_lang = (lang or "en").strip() or "en"
...
def get_facebook_thumbnail_binary(raw_url: str) -> Tuple[bytes, str]:
    if not isinstance(raw_url, str) or not raw_url.strip():
        raise FacebookTranscriptError(
            "INVALID_URL",
...
def build_internal_error() -> FacebookTranscriptError:
    return FacebookTranscriptError(
        "INTERNAL_ERROR",
        "Unexpected server error while processing Facebook transcript request.",
        500,
    )
